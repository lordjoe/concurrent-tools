...
/**
 * org.systemsbiology.hadoop.ClusterLauncher
 *
 * @author Steve Lewis
 * @date 2/27/14
 */
...
public class ClusterLauncher implements IStreamOpener { //extends AbstractParameterHolder implements IParameterHolder {
    ...
    private IParameterHolder m_Application;

    private final HadoopStatistics m_Statistics = new HadoopStatistics();
    ...
    public HadoopStatistics getStatistics() {
        return m_Statistics;
    }
    ...
    public boolean isDataOnCluster() {
        HadoopUtilities application = getApplication();
    ...
    public String getOutputFileName() {
        if (m_OutputFileName == null) {
            IParameterHolder application = getApplication();
    ...
    public void handleInputs(final InputStream is, Configuration cfg) {
        ...
        m_Application = IParameterHolder.getInstance(is, cfg);
...
//
    ...
    public void runJobs(IHadoopController launcher) {
        ...
        HadoopStatistics statistics = getStatistics();
...
//        pAccessor.writeToFileSystem(remotepath, sw.toString());
    ...
    public IParameterHolder getApplication() {
    ...
    protected String[] buildJobStrings(String... addedDefinitions) {
        ...
        holder.add("-D");
        holder.add(HadoopUtilities.PARAMS_KEY + "=" + taskParamsPath);
        holder.add("-D");
        holder.add(HadoopUtilities.PATH_KEY + "=" + remoteBaseDirectory);
        holder.add("-D");
        holder.add(HadoopUtilities.HOST_KEY + "=" + remoteHost);
        holder.add("-D");
        holder.add(HadoopUtilities.HOST_PORT_KEY + "=" + p);
//        Properties properties = HadoopUtilities.getHadoopProperties();
//        for (String key : properties.stringPropertyNames()) {
//            holder.add("-D");
//            String val = properties.getProperty(key);
...
//    }
    ...
    protected static void handleConfigurationFile(final String pCnfigfile) {
        InputStream describedStream = HadoopUtilities.getDescribedStream(pCnfigfile);
    ...
    protected static void handleValue(final String pProperty, String pValue) {
        ...
        if (MAX_REDUCE_TASKS_PROPERTY.equals(pProperty)) {
            ...
            HadoopUtilities.setMaxReduceTasksX(value);  // todo fix
    ...
    public static void workingMain(String[] args, IJobBuilderFactory builder) {
        ...
        try {
            ...
            IParameterHolder application = main.getApplication();
...
//            }
            ...
            XMLUtilities.outputLine();
            HadoopStatistics stats = main.getStatistics();
    ...
    public static void main(final String[] args) throws Exception {
        // reset globals
       // IParameterHolder.resetInstance();
